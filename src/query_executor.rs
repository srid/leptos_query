...
pub(crate) fn synchronize_state<K, V>(query: Signal<Query<K, V>>, executor: Rc<dyn Fn()>)
where
...
fn ensure_not_stale<K: Clone, V: Clone>(query: Signal<Query<K, V>>, executor: Rc<dyn Fn()>) {
...
fn ensure_not_invalid<K: Clone, V: Clone>(state: Signal<Query<K, V>>, executor: Rc<dyn Fn()>) {
    create_isomorphic_effect(move |_| {
        let state = state.get();
...
fn sync_refetch<K, V>(query: Signal<Query<K, V>>, executor: Rc<dyn Fn()>)
where
    K: Clone + 'static,
    V: Clone + 'static,
{
...
fn sync_observers<K: Clone, V: Clone>(query: Signal<Query<K, V>>) {
    ...
    // Ensure that observers are kept track of.
    create_isomorphic_effect(move |observers: Option<Rc<Cell<usize>>>| {
...
fn ensure_cache_cleanup<K, V>(query: Signal<Query<K, V>>)
where
    K: Clone + Hash + Eq + PartialEq + 'static,
    V: Clone + 'static,
{
